	...
	var $window = $( window ),
			...
			init = function( settings, callback ) {
				...
				_initMasonry( function() {
					...
					Gamma.container.removeClass( 'js-gallery__container--loading' );
					// show items
					Gamma.items.show();
			...
			_config = function() {
				...
				Gamma.container = $( '#js-gallery__container' );
				Gamma.overlay = Gamma.container.find( 'div.js-gallery__overlay' );
				Gamma.controls = Gamma.container.children( 'div.gamma-options' );
				Gamma.gallery = Gamma.container.children( 'ul.js-gallery__list' );
	...
	_svResizeImage = function( callback ) {
		...
		setTimeout( function() {
			...
			_applyAnimation( Gamma.overlay, { 'opacity' : 1 }, Gamma.settings.speed, Gamma.supportTransitions || !anim, function() {
				...
				$wrapper.addClass( 'overflow-none' );
		...
		var loadingtimeout = setTimeout( function() { Gamma.singleview.addClass( 'js-gallery__container--loading' );	}, Gamma.settings.svImageTransitionSpeedFade + 250 );
		...
		Gamma.svImage = $( '<img/>' ).load( function() {
			...
			Gamma.singleview.removeClass( 'js-gallery__container--loading' );
		...
		$wrapper.removeClass( 'overflow-none' );
